...
df_n = df_n.withColumn("EMPRESA",
                       F.when(df_n.EMPRESA == "ABJ", "AEROTÁXI ABAETÉ").
                       when(df_n.EMPRESA == "GLO", "GOL").
                       when(df_n.EMPRESA == "TAM", "LATAM BRASIL").
                       when(df_n.EMPRESA == "AZU", "AZUL").
                       when(df_n.EMPRESA == "PTB", "PASSAREDO")
                       )
...
setEmpresa = F.udf(lambda row: empresasInternacional.get(row, row))

...
url = "jdbc:postgresql://localhost:5432/staged"
...
df_n = spark.read.jdbc(
    url=url, table="nacional", properties=properties)
df_i = spark.read.jdbc(
    url=url, table="internacional", properties=properties)
...
df_n = df_n.withColumn("CLASSE_IDA", F.lit(None))
df_n = df_n.withColumn("CLASSE_VOLTA", F.lit(None))

df_n = df_n.select("ANO", "MES", "ICAO", "EMPRESA", "CLASSE_IDA",
                   "CLASSE_VOLTA", "ORIGEM", "PAIS_ORIGEM",
...
df_i = df_i.select("ANO", "MES", "ICAO", "EMPRESA", "CLASSE_IDA",
                   "CLASSE_VOLTA", "ORIGEM", "PAIS_ORIGEM",
                   "CIDADE_ORIGEM", "DESTINO", "PAIS_DESTINO",
                   "CIDADE_DESTINO", "TARIFA", "ASSENTOS")
...
df = df.select("ID", "ANO", "MES", "ICAO", "EMPRESA", "CLASSE_IDA",
               "CLASSE_VOLTA", "ORIGEM", "PAIS_ORIGEM",
               "CIDADE_ORIGEM", "DESTINO", "PAIS_DESTINO",
...
df.write.format("jdbc")\
    .mode("overwrite")\
    .option("url", "jdbc:postgresql://localhost:5432/dwarehouse") \
    .option("driver", "org.postgresql.Driver")\
    .option("dbtable", "anac_warehouse") \
